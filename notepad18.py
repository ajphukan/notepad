...
#  Copyright 2016 Paul Sutton <psutton@zleap.net>
#  
...
def open_command():
	global filename # We'll need the filename later, when we save
	
	txt.delete(1.0, END) # Clear the text field
	
	filename = tkFileDialog.askopenfilename(parent=window, title='Select a file') # Get a filename
	
	if filename:
		textfile = open(filename, 'r') # Open the file
		txt.insert(END, textfile.read()) # Fill the text field
...
def save(filename):
	if filename:
		...
		textfile.write(txt.get('1.0', END+'-1c'))
		textfile.close()
